...
export default function App() {
    ...
    reader.onload = e => {
      try {
        ...
        const mapped = parsed.map(r => ({
          생산계획일자: fmtXlDate(r["생산계획 일자"] || r["생산계획일"] || r["생산일자"] || r["생산일"] || r["계획일"]),
  ...
  // 💡 데이터 정렬 로직 (빈칸은 항상 맨 아래로)
  const sortedShip = useMemo(() => {
    return [...filteredShip].sort((a, b) => {
      const vA = a.납기일자; const vB = b.납기일자;
      if (!vA && !vB) return 0;
      if (!vA) return 1; if (!vB) return -1;
      return sortDesc ? vB.localeCompare(vA) : vA.localeCompare(vB);
    });
  }, [filteredShip, sortDesc]);
  ...
  const renderDashView = () => (
    <>
      ...
      {/* 💡 검색 바 & 정렬 버튼 추가 */}
      <div style={{ display: "flex", gap: 8, padding: "14px 16px 0", maxWidth: "100%" }}>
            ...
            <div className="swipe-menu">
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "550px" }}>
                ...
                <tbody>
                  ...
                  {sortedProd.length === 0 && <tr><td colSpan="6" style={{ ...TD, textAlign: "center", padding: "30px", color: "#94a3b8" }}>{search ? "검색 결과가 없습니다." : "데이터가 없습니다."}</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
  ...
  return (
    <>
      ...
      <div style={{ fontFamily: "'Pretendard','Malgun Gothic',sans-serif", background: "#f1f5f9", minHeight: "100vh" }}>
        ...
        {/* 💡 컴포넌트 렌더링 방식 변경으로 검색창 포커스 버그 완벽 해결 */}
        {view === "input" ? renderInputView() : renderDashView()}
      </div>
    </>
  );
}